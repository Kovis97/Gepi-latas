...
for e in dirlist:
    print(dirname + '/'+ e.name)
    kepe = cv2.haveImageReader(dirname + '/'+ e.name)
    if (not kepe):
        print('\tEz nem kep')
    else:
        I = cv2.imread(dirname + '/'+ e.name)
        ...
        text = (e.name + '\t' + str(korok) + '\t' + str(kocka) + '\n')
        ...
        cv2.imshow("detected circles on: " + str(e.name),I_k)
